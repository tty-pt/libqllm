...
static int32_t _mock_n_head = 32;
static int32_t _mock_n_head_kv = 32;
static int32_t _mock_vocab_size = 32000;
static const char * _mock_chat_template = "{{prompt}}";
...
static int _grammar_init_count = 0;
...
void
mock_llama_set_n_head(int32_t n_head)
{
	_mock_n_head = n_head;
}

void
mock_llama_set_n_head_kv(int32_t n_head_kv)
{
	_mock_n_head_kv = n_head_kv;
}

void
mock_llama_set_chat_template(const char *tmpl)
{
	_mock_chat_template = tmpl;
}
...
int
mock_llama_get_grammar_init_count(void)
{
    return _grammar_init_count;
}
...
int32_t
llama_model_n_head(const struct llama_model *model)
{
	(void)model;
	return _mock_n_head;
}
...
int32_t
llama_model_n_head_kv(const struct llama_model *model)
{
	(void)model;
	return _mock_n_head_kv;
}

const char *
llama_model_chat_template(const struct llama_model *model, const char *name)
{
	(void)model;
	(void)name;
	return _mock_chat_template;
}

int32_t
llama_chat_apply_template(const char *tmpl, const struct llama_chat_message *chat,
			  ...
			  char *buf, int32_t length)
{
	...
	(void)tmpl;
	for (i = 0; i < n_msg; i++) {
		const char *src = chat[i].content;

		if (src) {
			while (*src && used < (size_t)length - 1)
				buf[used++] = *src++;
		}
	}
	if (add_ass) {
...
void
llama_memory_clear(llama_memory_t memory, bool editing)
{
	struct llama_context *ctx = (struct llama_context *)memory;
